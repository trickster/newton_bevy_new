        ...
        .with_children(|u| {
            ...
            for _ in 0..20 {
                ...
                u.spawn(PbrBundle {
                    mesh: sphere_handle.clone(),
                    material: sphere_material_handle.clone(),
                    transform: Transform::from_scale(Vec3::splat(radius)).with_translation(
                        Vec3::new(
                            rng.gen::<f32>() * 5.0 - 2.5,
                            rng.gen::<f32>() * 5.0 - 2.5,
                            rng.gen::<f32>() * 5.0 - 2.5,
...
fn update_camera(
    ...
    for event in mouse_motion_events.iter() {
        ...
        forward =
            Quat::from_axis_angle(left, event.delta.y * time.delta_seconds()).mul_vec3(forward);
        forward =
            Quat::from_axis_angle(down, event.delta.x * time.delta_seconds()).mul_vec3(forward);
...
fn collision_system(
    query: Query<(Entity, &Transform, &Radius)>,
    mut collisions: EventWriter<Collision>,
) {
    for [(a_id, a_transform, &Radius(a_radius)), (b_id, b_transform, &Radius(b_radius))] in
...
fn collision_handler_system(
    mut bodies: Query<(&mut Transform, &mut Velocity, &Radius, &Mass)>,
    mut collisions: EventReader<Collision>,
) {
    for &Collision(a_id, b_id) in collisions.iter() {
        let (a_transform, a_velocity, &Radius(a_radius), &Mass(a_mass)) = bodies.get(a_id).unwrap();
        let (b_transform, b_velocity, &Radius(b_radius), &Mass(b_mass)) = bodies.get(b_id).unwrap();
        let v = a_transform.translation - b_transform.translation;
        let d = a_radius + b_radius - v.length();
        let dir = v.normalize();
        ...
        let new_a_velocity = Velocity(
            a_velocity.0
                - (2.0 * b_mass / (a_mass + b_mass))
                    * ((a_velocity.0 - b_velocity.0).dot(v) / ds)
                    * v,
        );
        let new_b_velocity = Velocity(
            b_velocity.0
                - (2.0 * a_mass / (a_mass + b_mass))
                    * ((b_velocity.0 - a_velocity.0).dot(-v) / ds)
                    * -v,
        );
...
const G: f32 = 1.0;
fn gravity_system(time: Res<Time>, mut query: Query<(&Transform, &mut Velocity, &Mass)>) {
    let mut combinations = query.iter_combinations_mut();
    while let Some(
        [(a_transform, mut a_velocity, &Mass(a_mass)), (b_transform, mut b_velocity, &Mass(b_mass))],
